...
export const getOutOfScope = (split) => {
...
const getModuleFromTree = (line, currentDir) => {
    line = line.trim();
    ...
    if (isDir) {
        ...
        if (!currentDir || depth <= prevDepth)
            return { undefined, currentDir: dirName };
        ...
        if (currentDir)
            return { module: `${currentDir}/${module}`, currentDir };
        else
            return { module, currentDir };
    }
};
const getInScopeLines = (inScopeStart, split, inScope, scope) => {
    const scopeStartEnd = (line) => line.match(/^```.*$/g);
...
//# sourceMappingURL=getOutOfScope.js.map