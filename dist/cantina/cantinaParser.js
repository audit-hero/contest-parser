...
export const parseActiveCantinaContests = async (existingContests) => await pipe(TE.fromTask(() => getActiveContests()), log((active) => `cantina: active contests: ${active.map((it) => it.name).join(", ")}`), TE.map((active) => active.filter((it) => {
