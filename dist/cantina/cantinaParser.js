...
export const parseActiveCantinaContests = async (existingContests) => await pipe(TE.fromTask(() => getActiveContests()), TE.map((it) => it.filter((it) => it.name.includes("bloom"))), log((active) => `cantina: active contests: ${active.map((it) => it.name).join(", ")}`), TE.map((active) => active.filter((it) => {
    let existing = existingContests.find((existing) => existing.pk === it.name);
    return !existing || existing.modules?.length === 0;
})), TE.chain((it) => sequence(TE.ApplicativePar)(it.map((it) => TE.fromTask(() => parseContest(it))))), TE.getOrElse((e) => {
    Logger.error(e);
