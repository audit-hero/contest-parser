...
import fs from "fs";
let contestObj = { start_time: "September 26, 2022 20:00 UTC", end_time: "September 29, 2022 20:00 UTC", amount: "100$" };
test("parses correct contracts", () => {
    var md = fs.readFileSync(`${workingDir}/src/c4/test/correct-contracts.md`).toString();
...
test("handles readme with invalid contracts ", () => {
    var md = fs.readFileSync(`${workingDir}/src/c4/test/invalid-contracts.md`).toString();
    let contest = parseMd("url", md, "repo", contestObj).value;
...
it("parses slash-ending urls", async () => {
    var md = fs.readFileSync(`${workingDir}/src/c4/test/c4-slash-ending-url.md`).toString();
    let parsed = findModules("repo", md.split("\n"), 0);
