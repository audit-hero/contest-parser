...
export const getActiveC4Contests = async () => {
    ...
    let contests = pipe(parseMd(md), E.map((it) => truncateLongNames(it)));
    return contests;
};
let parseMd = (md) => pipe(E.Do, E.bind("start", () => pipe(O.fromNullable(md.match(/^#{1,3} .*(A|a)ctive/m)?.index), E.fromOption(() => "no active contests found"))), E.bind("end", ({ start }) => pipe(O.fromNullable(md.slice(start).match(/^#{1,3}/m)?.[0].length), E.fromOption(() => "no end found"), E.map((hashCount) => start + md.slice(start).indexOf(`\n${"#".repeat(hashCount)} `, 1)))), E.map(({ start, end }) => parseActive(md, { start, end })), E.mapLeft((it) => new Error(it)));
// prettier-ignore
