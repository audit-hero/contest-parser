...
export const parseActiveHackenContests = async (existingContests) => await pipe(TE.fromTask(() => getActiveContests()), log((active) => `hacken: active contests: ${active.map((it) => it.name).join(", ")}`), TE.map((active) => active.filter((it) => {
    let existing = existingContests.find((existing) => existing.pk === it.name);
    return !existing || existing.modules?.length === 0;
})), TE.chain((contests) => pipe(contests.map((contest) => TE.fromTask(() => parseContest(contest))), sequence(TE.ApplicativePar))), 
